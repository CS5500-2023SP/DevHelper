...
public class CloudCreateTableAndImportData {
  ...
  public static void createTable(DynamoDbClient client) {
    CreateTableRequest request = CreateTableRequest.builder().
        ...
        provisionedThroughput(ProvisionedThroughput.builder().readCapacityUnits(20L)
            .writeCapacityUnits(20L).build())
  ...
  public static void importData(DynamoDbClient client, String file)
    ...
    for (JsonNode activity : jsonNode) {
      ...
      while (!success) {
        try {
          ...
          System.err.println("Failed to write item to DynamoDB: " + e.getMessage());
          // Optionally, you could add a delay before retrying to avoid overwhelming the database
        }
        System.out.println("[SLEEP IN WHILE LOOP]");
        Thread.sleep(2000);
      }
      System.out.println("[SLEEP IN FOR LOOP]");
      Thread.sleep(2000);
    }
  }
}
